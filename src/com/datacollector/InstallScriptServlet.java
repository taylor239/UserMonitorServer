...
public class InstallScriptServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if(curDevice.equals("debvm") || curDevice.equals("debrpi"))
		{
			...
			+ "\nsudo -E dpkg --configure -a >> installOutput.txt" 
			+ "\nsudo -E apt-get -y clean >> installOutput.txt" 
			+ "\nsudo rm /var/lib/dpkg/lock >> installOutput.txt"
			+ "\nsudo rm /var/cache/apt/archives/lock >> installOutput.txt"
			+ "\nsudo -E dpkg --configure -a >> installOutput.txt" 
			+ "\nsudo -E apt-get -y update --fix-missing >> installOutput.txt"
			+ "\nif [[ $? > 0 ]]"
			+ "\nthen"
			...
			+ "\nsudo -E apt-get -y install chrony >> installOutput.txt"
			+ "\nif [[ $? > 0 ]]"
			+ "\nthen"
			...
			+ "\n\tsudo -E systemctl start chrony >> installOutput.txt"
			+ "\n\tsudo -E systemctl enable chrony >> installOutput.txt"
			+ "\nsudo -E chronyd -q >> installOutput.txt"
			+ "\nfi"
			+ "\nif [[ $? > 0 ]]"
			+ "\nthen"
			+ "\n\techo \"Warning: chrony sync failed, attempting to sync with other tools but assuming clock is accurate already\""
			+ "\n\tsudo -E timedatectl set-ntp True >> installOutput.txt"
			+ "\n\tsudo -E timedatectl >> installOutput.txt"
			+ "\n\tsudo -E apt-get -y install ntpdate >> installOutput.txt"
			+ "\n\tsudo -E /etc/init.d/ntp stop >> installOutput.txt"
			+ "\n\tsudo -E ntpd -q -g >> installOutput.txt"
			+ "\n\tsudo -E /etc/init.d/ntp start >> installOutput.txt"
			//+ "\n\tsuccessfulInstall=false"
			+ "\nelse"
			+ "\n\techo \"chrony sync successful\""
			+ "\nfi"
			+ "\necho \"Installing wget\""
			+ "\nsudo -E apt-get -y install wget >> installOutput.txt" 
			+ "\nif [[ $? > 0 ]]"
			+ "\nthen"
			...
			+ "\nsudo -E add-apt-repository -y ppa:linuxuprising/java >> installOutput.txt" 
			+ "\nsudo -E apt-get -y update >> installOutput.txt" 
			+ "\nsudo -E apt-get -y install oracle-java16-installer" 
			+ "\nif [[ $? > 0 ]]"
			+ "\nthen"
			...
			//+ "\n\tsuccessfulInstall=false"
			+ "\n\tsudo -E apt-get -y install default-jre >> installOutput.txt" 
			+ "\n\tif [[ $? > 0 ]]"
			+ "\n\tthen"
			...
			+ "\nsudo -E apt-get -y install mysql-server >> installOutput.txt" 
			+ "\nif [[ $? > 0 ]]"
			+ "\nthen"
			+ 		"\n\techo \"Warning: MySql install failed, trying MariaDB\""
			+ 		"\n\tsudo -E apt-get -y install mariadb-server >> installOutput.txt" 
			+ 		"\n\tif [[ $? > 0 ]]"
			+ 		"\n\tthen"
			...
			+ "\nsudo -E apt-get -y install tomcat8 >> installOutput.txt" 
			+ "\nif [[ $? > 0 ]]"
			+ "\nthen"
			+ 		"\n\techo \"Warning: version 8 install failed, trying version 9\""
			+ 		"\n\tsudo -E apt-get -y install tomcat9 >> installOutput.txt" 
			+ 		"\n\tif [[ $? > 0 ]]"
			+ 		"\n\tthen"
			...
			+ "\nsudo -E systemctl enable mysql >> installOutput.txt"
			+ "\nsudo -E systemctl start mysql >> installOutput.txt"
			+ "\n\nsudo -E service mysql start >> installOutput.txt"
			+ "\nsudo -E update-rc.d mysql enable >> installOutput.txt"
			...
			+ "\n\nsudo mysql -u root -e \"DROP DATABASE IF EXISTS dataCollection;\" >> installOutput.txt"
			...
			//+ "\nservice mysql start"
			+ "\nsudo -E chronyd -q >> /opt/dataCollector/startupOutput.txt"
			+ "\nsudo -E systemctl enable mysql >> /opt/dataCollector/startupOutput.txt"
			+ "\nsudo -E systemctl start mysql >> /opt/dataCollector/startupOutput.txt"
			+ "\nsudo -E service mysql start >> /opt/dataCollector/startupOutput.txt"
			+ "\nwhile true;" 
			+ "\ndo" 
